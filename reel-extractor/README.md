# Instagram Reel Knowledge Extractor — Custom GPT Action
...
1. ChatGPT → **Explore GPTs → Create** (requires ChatGPT Plus).
2. **Configure** tab → name it (e.g. "Reel Scholar") → paste the contents of
   `gpt_instructions.md` into **Instructions**.
3. Scroll to **Actions → Create new action**.
4. Paste the contents of `openapi.yaml` into the schema box, replacing
...
5. **Authentication → API Key** → Auth type "Custom" → header name `X-API-Key`
   → paste the same value you set as `REEL_API_KEY`.
6. Save. Test by pasting a reel link into the GPT's chat.
